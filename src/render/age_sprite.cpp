...
int ASprite::touchState()
{
    return this->touchState();
}

void ASprite::dumpTouchState()
{
    if(this->isTouchable())
    {
        this->touch_state=AGE_SPRITE_TOUCH_NULL;
    }else
    {
        this->touch_state=AGE_SPRITE_TOUCH_NOT_ACTIVE;
    }
}
...
int ASprite::isHover(AVector2D pos)
{
    APolygon polygon;
    polygon.addPoint(this->leftBotton());
    polygon.addPoint(this->leftTop());
    polygon.addPoint(this->rightTop());
    polygon.addPoint(this->rightBotton());

    return polygon.pointInPolygon(pos);
}
...
void ASprite::setSize(float width, float height)
{
    setWidth(width);
    setHeight(height);
}
