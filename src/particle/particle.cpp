...

{
    ...
    //qDebug()<<"begin angle"<<this->begin_angle;
...
void Particle::action()
{
    ...
    {
        this->setWidth(this->width()+width_change_rate*ASystem::TimeStep());
        this->setHeight(this->height()+height_change_rate*ASystem::TimeStep());
    }


    //颜色变化
    AColor current_color=this->getColor();
    this->setColor(current_color.red+red_change_rate*ASystem::TimeStep(),
                   current_color.green+green_change_rate*ASystem::TimeStep(),
                   current_color.blue+blue_change_rate*ASystem::TimeStep()
                   );

    //不透明度变化
    this->setAlpha(this->getAlpha()+alpha_change_rate*ASystem::TimeStep());
    this->setPivotOffset(this->width()/2,this->height()/2);
    angle+=rotate_speed*ASystem::TimeStep();
    this->rotate(angle);
