...
void *ASpriteCreate()
{
    ASpriteActor * sprite =new ASpriteActor();
    return sprite;
}
...
{
    ASpriteActor * sprite= (ASpriteActor* )sprite_handle;
...
{
    ASpriteActor * sprite = (ASpriteActor *) sprite_handle;
    AAnimationSet * animationSet = (AAnimationSet*)animationSet_handle;
    sprite->bindAnimationSet(animationSet);
}

static int ASpriteBindAnimationSet_bind(lua_State *L)
{
    if(lua_islightuserdata(L,1) && lua_islightuserdata(L,2))
    {
        ASpriteBindAnimationSet(lua_touserdata(L,1),lua_touserdata(L,2));
    }
    else
    {
        AGE_LUA_ADD_DEBUGEDIT("void ASpriteBindAnimationSet(void *sprite_handle, void *animationSet_handle)");
    }
...
{
    ASpriteActor * sprite = (ASpriteActor *) sprite_handle;
    sprite->animate(action,frame,isCycle);
}

static int ASpritePlay_bind(lua_State *L)
{
    if(lua_islightuserdata(L,1) && lua_isstring(L,2) && lua_isnumber(L,3) && lua_isboolean(L,4))
    {
        ASpritePlay(lua_touserdata(L,1),lua_tostring(L,2),lua_tonumber(L,3),lua_toboolean(L,4));
    }
    else
    {
        AGE_LUA_ADD_DEBUGEDIT("void ASpritePlay(void *sprite_handle, const char *action, int frame, bool isCycle)");
    }
...
{
    AAnimationSet * m_animationSet = new AAnimationSet(xmlFileName);
    return (void *)m_animationSet;
}

static int AAnimationSetCreate_bind(lua_State *L)
{
    if(lua_isstring(L,1))
    {
        void * AnimationSet = AAnimationSetCreate(lua_tostring(L,1));
        lua_pushlightuserdata(L,AnimationSet);
    }
    else
    {
...
static int AGetSystem_bind(lua_State * L)
{
    lua_pushstring(L,AGetSystem());
    return 1;
}
...
{
    ASystem::GetWidget()->setWindowTitle(title);
}


static int ASetTitle_bind(lua_State * L)
{
    if(lua_isstring(L,1))
    {
        ASetTitle(lua_tostring(L,1));
    }
    return 0;
}
...
static int AClock_bind(lua_State * L)
{
    lua_pushinteger(L,AClock());
    return 1;
}
...
int ASpriteHover(void *sprite_handle, int x, int y)
{
    ...
    return sprite->isHover(AVector2D(x,y));
}

static int ASpritrHover_bind(lua_State * L)
{
    if(lua_isuserdata(L,1)&&lua_isnumber(L,2)&&lua_isnumber(L,3))
    {
        int result= ASpriteHover(lua_touserdata(L,1),lua_tointeger(L,2),lua_tointeger(L,3));
...
void ARegisterToLua()
{
    ...
    lua_register(ALua::getLua(),"ASpritePlay",ASpritePlay_bind);
    lua_register(ALua::getLua(),"ASpriteBindAnimationSet",ASpriteBindAnimationSet_bind);
    lua_register(ALua::getLua(),"ASpriteHover",ASpritrHover_bind);
    lua_register(ALua::getLua(),"AAnimationSetCreate",AAnimationSetCreate_bind);
    ...
    lua_register(ALua::getLua(),"AGetSystem",AGetSystem_bind);
    lua_register(ALua::getLua(),"ASetTitle",ASetTitle_bind);
    lua_register(ALua::getLua(),"AClock",AClock_bind);
